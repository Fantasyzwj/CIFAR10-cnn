...
patchDim = 17;          % patch dimension,卷积后图像变为16*16
...
poolDim = 4;          % dimension of pooling region，池化后图像变为4*4
...
load data/trainData.mat % loads numTrainImages, trainImages, trainLabels
trainImages = reshape(trainImages,32,32,3,50000);
trainImages = trainImages(:,:,:,1:10000);
...
load data/trainData.mat % loads numTrainImages, trainImages, trainLabels
load data/testData.mat  % loads numTestImages,  testImages,  testLabels

trainImages = reshape(trainImages,32,32,3,50000);
trainImages = trainImages(:,:,:,1:10000);
numTrainImages = 10000;

testImages = reshape(testImages,32,32,3,10000);
testImages = testImages(:,:,:,1:5000);
numTestImages = 5000;
...
for convPart = 1:(hiddenSize / stepSize)
    ...
    toc();
    clear convolvedFeaturesThis pooledFeaturesThis;% 清空中间变量
